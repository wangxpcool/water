...
public class AssetAccountCommandService {
    ...
    private void bindStatement(PreparedStatement statement, AssetAccountUpsertRequest request) throws java.sql.SQLException {
        ...
        statement.setString(3, request.accountType().trim().toUpperCase(Locale.ROOT));
        statement.setString(4, request.balanceDirection().trim().toUpperCase(Locale.ROOT));
        statement.setString(5, request.currencyCode().trim().toUpperCase(Locale.ROOT));
        statement.setString(6, blankToNull(request.institutionName()));
        statement.setString(7, blankToNull(request.ownerName()));
        statement.setString(8, blankToNull(request.remark()));
        statement.setInt(9, request.sortOrder());
        statement.setInt(10, request.enabled() ? 1 : 0);
    }
